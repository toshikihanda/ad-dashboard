...
import { updateCandidateStatus, appendToKnowledge } from '@/lib/candidateSheets';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { candidate_id, decision, comment } = body;
    ...
    const updated = await updateCandidateStatus(candidate_id, status, comment || '');
