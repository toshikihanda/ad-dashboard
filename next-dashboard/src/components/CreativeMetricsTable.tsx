...
function normalizeCreativeLookupId(value: string): string {
...
export function CreativeMetricsTable({ data, title = 'クリエイティブ別数値', creativeMasterData, isReport = false }: CreativeMetricsTableProps) {
    ...
    const getPreviewUrl = (campaign: string, creativeId: string) => {
        ...
        const normalizedCreativeId = normalizeCreativeLookupId(creativeId);
        if (!normalizedCreativeId) return null;

        const campaignCandidates = creativeMasterData.filter(item => {
            const itemBaseCamp = item.campaign.split('_')[0];
            return itemBaseCamp === baseCampaign;
        });

        if (campaignCandidates.length === 0) return null;

        // 1. 完全一致（219b は 219b にだけ紐づける）
        const exactMatch = campaignCandidates.find(item =>
            normalizeCreativeLookupId(item.creativeId) === normalizedCreativeId
        );
        ...
        const exactFileMatch = campaignCandidates.find(item => {
            ...
            return normalizedFileName === normalizedCreativeId;
        });
